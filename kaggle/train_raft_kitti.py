"""
Fine-tune RAFT-Stereo (realtime config) on KITTI 2015 — Kaggle version.

Self-contained. Reproduces the thesis protocol:
  - 200 training pairs (*_10.png reference frames), sorted order
  - seed-42 random split, val_split=0.10 → same 20 val samples as AANet runs
  - D1/EPE metrics identical to the thesis ablation table

Usage (inside Kaggle notebook, after cloning RAFT-Stereo — see README.md):
    python train_raft_kitti.py \
        --data-root /kaggle/input/YOUR-KITTI-DATASET/data_scene_flow \
...
class KITTI2015(Dataset):
    """Matches the thesis loader: *_10.png frames from training/, sorted.
    Disparity: 16-bit PNG / 256.0, zero = invalid."""

    def __init__(self, root, crop=None, training=True):
        self.root = Path(root)
        self.crop = crop
        self.training = training
        left_dir = self.root / "training" / "image_2"
        self.lefts = sorted(left_dir.glob("*_10.png"))
        if not self.lefts:
            raise SystemExit(f"No '*_10.png' under {left_dir}")
    ...
    def __getitem__(self, i):
        lp = self.lefts[i]
        L = cv2.cvtColor(cv2.imread(str(lp)), cv2.COLOR_BGR2RGB)
        R = cv2.cvtColor(cv2.imread(str(self.root / "training" / "image_3" / lp.name)),
                         cv2.COLOR_BGR2RGB)
        D = cv2.imread(str(self.root / "training" / "disp_occ_0" / lp.name),
...
def main():
    ...
    p.add_argument("--out",       default="./ckpt_kitti")
    ...
    full_train = KITTI2015(args.data_root, crop=tuple(args.crop), training=True)
    full_val   = KITTI2015(args.data_root, crop=None, training=False)
    n = len(full_train)
    ...
    print(f"train {len(train_set)}  val {len(val_set)}  pairs")
